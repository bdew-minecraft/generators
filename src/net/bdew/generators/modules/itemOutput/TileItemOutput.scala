...
class TileItemOutput extends TileOutput[OutputConfigItems] with RSControllableOutput with InventoryProxy with ISidedInventory {
  ...
  override def makeCfgObject(face: ForgeDirection) = new OutputConfigItems
  ...
  override def canExtractItem(slot: Int, item: ItemStack, side: Int) =
    getCfg(Misc.forgeDirection(side)) exists checkCanOutput
  override def canInsertItem(slot: Int, item: ItemStack, side: Int) = false
}
